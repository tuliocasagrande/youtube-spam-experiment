...
class SingleClassification(BaseClassification):
    ...
    def __init__(self, filename, train_percent=0.7, test_percent=None,
        ...
        if stratified:
            ...
            self.y_train = np.concatenate([self.y_spam[:train_index_spam],
                                           self.y_ham[:train_index_ham]])

            self.X_test = np.concatenate([self.X_spam[test_index_spam:],
                                          self.X_ham[test_index_ham:]])
            self.y_test = np.concatenate([self.y_spam[test_index_spam:],
...
class DualClassification(BaseClassification):
    """ Dual supervised training
        1- train with the first oldest labeled comments (below train_percent)
        2- label more comments with the intermediate classifier (between
            train_percent and ss_percent)
        3- test final classifier with the remaning comments (above ss_percent)
        The dataset must be ordered by date (first = oldest)
    """

    def __init__(self, filename, threshold=None, train_percent=0.3,
                 ss_percent=0.4, stratified=True):
        ...
        if stratified:
            ...
            self.X_ss = np.concatenate([
                self.X_spam[train_index_spam:ss_index_spam],
                self.X_ham[train_index_ham:ss_index_ham]])
            self.y_ss = np.concatenate([
                self.y_spam[train_index_spam:ss_index_spam],
                self.y_ham[train_index_ham:ss_index_ham]])

            self.X_test = np.concatenate([self.X_spam[ss_index_spam:],
                                          self.X_ham[ss_index_ham:]])
            self.y_test = np.concatenate([self.y_spam[ss_index_spam:],
    ...
    def classify(self, interm_clf, final_clf):
        ...
        return(self.y_test, self.y_pred_final, interm_clf, final_clf,
               sum(above_threshold), len(self.X_ss))
    ...
    def prepare_final(self):
        if self.threshold:
            X_train = np.concatenate([self.X_train,
                                     self.X_ss[self.above_threshold]])
            y_train = np.concatenate([self.y_train,
                                     self.y_pred_interm[self.above_threshold]])
        else:
...
class SemiSupervisedClassification(DualClassification):
    """ Modify the DualClassification to use the LabelPropagation and
    LabelSpreading classifiers. Both algorithms require unlabeled data (-1)
